...
assert.ok(files['prj.conf'].includes('CONFIG_USB_DEVICE_STACK_NEXT=y'))
assert.ok(files['prj.conf'].includes('CONFIG_CDC_ACM_SERIAL_INITIALIZE_AT_BOOT=y'))
...
assert.ok(files['boards/xiao_ble.overlay'].includes('board_cdc_acm_uart'))
assert.ok(files['boards/xiao_ble.overlay'].includes('zephyr,uart-mcumgr = &board_cdc_acm_uart'))
assert.ok(files['sysbuild/mcuboot/boards/xiao_ble.overlay'].includes('slot0_partition'))
assert.ok(files['sysbuild/mcuboot/boards/xiao_ble.overlay'].includes('slot1_partition'))
assert.ok(files['sysbuild/mcuboot/boards/xiao_ble.overlay'].includes('boot_partition'))
assert.ok(!files['sysbuild/mcuboot/boards/xiao_ble.overlay'].includes('board_cdc_acm_uart'))
