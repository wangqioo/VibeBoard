...
assert.match(compilePanel, /import \{ createEvidencePackage, createEvidenceReportMarkdown \} from '..\/domain\/evidence\/evidencePackage'/)
assert.match(compilePanel, /function downloadTextFile\(/)
assert.match(compilePanel, /function createFirmwareArtifactSummary\(/)
assert.match(compilePanel, /function handleExportEvidenceReport\(/)
assert.match(compilePanel, /const \[latestDeviceEvidence, setLatestDeviceEvidence\] = useState\(null\)/)
...
assert.match(compilePanel, /createEvidencePackage\(\{[\s\S]*boardId,[\s\S]*selectedSkills: selectedSkills \|\| \[\],[\s\S]*manifest,[\s\S]*projectFiles: sourceProp \|\| \{\},[\s\S]*buildEvidence,[\s\S]*deviceEvidence: latestDeviceEvidence,[\s\S]*artifact: createFirmwareArtifactSummary\(firmware\),[\s\S]*\}\)/)
assert.match(compilePanel, /createEvidenceReportMarkdown\(pkg\)/)
assert.match(compilePanel, /vibeboard-esp32-evidence-/)
assert.match(compilePanel, /导出证据报告/)
...
assert.match(compilePanel, /编译、烧录与证据/)
assert.match(compilePanel, /编译证据/)
assert.match(compilePanel, /aria-label="编译证据"/)
assert.doesNotMatch(compilePanel, />Build Evidence</)
...
assert.match(compilePanel, />\s*导出修复上下文\s*</)
